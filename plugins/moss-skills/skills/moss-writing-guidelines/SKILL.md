...
Match the representation to the information's shape, hierarchy, and density. "Default to prose" is the wrong instinct: pick the node that lets a reader scan the structure fastest at the level of detail they need.
...
| 2-4 alternate views, options, personas, or states sharing one context | Tabs | Not when side-by-side comparison matters — use a table or matrix. Tabs can hold full editor content (tables, callouts, code, HTML). |
...
| A rough diagram where spatial layout matters more than the text | Canvas | Preserve existing canvas; create `moss-canvas` only when the user asks or layout is the point. Otherwise use ASCII. |
...
When two nodes fit equally, choose the one that reads faster.
...
- Product states can live in a visual HTML state board. Keep risks, decisions, mitigations, and open questions as editable/commentable markdown tables or callouts unless the visual itself is the deliverable.
...
- Each piece of content uses the representation that fits its shape, hierarchy, and density.
