...
5. Define objective, non-goals, owned files, forbidden files, dependencies, acceptance criteria, validation, and review risk.
6. Include Moss-specific invariants from `AGENTS.md` or `CLAUDE.md` for Lexical, Jotai, IPC, markdown layers, storage, or note-switch behavior when affected.
7. **Search for existing precedence before designing new effects.** If the work would introduce a new write, IPC channel, migration, lifecycle hook, transformer, decorator, atom, fs walk, or similar effect class, first grep the codebase for an existing framework that already handles that effect class (e.g. `note-markdown-migration-runner.ts` for markdown migrations, `ipcChannels.ts` for IPC, `tokens.css` for color tokens). The default is to register with the existing framework. Inventing a new path requires an explicit documented justification (what's wrong with the existing pattern for this case) in the plan.
8. Require manager approval before implementation for medium, risky, cross-file, or user-facing work.
